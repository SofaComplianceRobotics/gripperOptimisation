...
def tangent_from_handles(pt: PincerSplinePoint) -> Vector:
    """
    Compute the tangent vector at one anchor point from its handles.

    Inputs:
        pt (PincerSplinePoint): Point record with anchor and handles.

    Returns:
        cadquery.Vector: Tangent vector at the anchor.
    """
    p = pt.p
    h_in = pt.h_in
    h_out = pt.h_out
...
def offset_wire(wire: Wire, dist: float) -> Wire:
    """
    Offset a wire by a given distance.

    Inputs:
        wire (cadquery.Wire): Source wire.
        dist (float): Offset distance.

    Returns:
        cadquery.Wire: Offset wire.

    Raises:
        ValueError: If the offset cannot be built because it is too large
            for the tightest bend in the curve.
    """
    from OCP.BRepOffsetAPI import BRepOffsetAPI_MakeOffset
    from OCP.GeomAbs import GeomAbs_Arc

    tool = BRepOffsetAPI_MakeOffset()
    tool.Init(GeomAbs_Arc)
    tool.AddWire(wire.wrapped)

    try:
        tool.Perform(dist)

        if hasattr(tool, "IsDone") and not tool.IsDone():
            raise ValueError
        ...
        raise ValueError(
            f"Offset of {dist:.2f} failed — pincer_profile_width is too large for "
            "the tightest bend in your curve. Reduce pincer_profile_width or "
            "soften the curve."
        ) from exc
