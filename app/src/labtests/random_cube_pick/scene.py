...
What this file owns:
  ...
  - SHAPEOPT_FINISH_BONUS
  - Its own recording path
  - PlaybackController (identical logic to grasp_hold except end-of-frames)
  - createScene() wiring
...
# ── random_cube_pick-specific config ──────────────────────────────────────────
...
def _resolve_cube_config() -> tuple[list[float], float]:
    """Return (cube_scale, cube_mass) for this run slot and generation."""
    cycle_index = OPTUNA_RUN_SLOT % 3
    cube_scale = list(_CUBE_SIZE_CYCLE[cycle_index])

    lo = min(SHAPEOPT_CUBE_WEIGHT_MIN, SHAPEOPT_CUBE_WEIGHT_MAX)
    hi = max(SHAPEOPT_CUBE_WEIGHT_MIN, SHAPEOPT_CUBE_WEIGHT_MAX)
    cube_mass = random.Random(OPTUNA_GEN).uniform(lo, hi)

    print(
        f"[cube] random_cube_pick slot={OPTUNA_RUN_SLOT} gen={OPTUNA_GEN} "
...
def createScene(rootnode):
    ...
    class PlaybackController(Sofa.Core.Controller):
        ...
        def _update_overload_mass(self) -> None:
            # Mass is fixed for random_cube_pick — no ramp, keeps the draw fair
            # across runs. Just ensure the value is set correctly each frame.
            self._set_cube_mass(cube_mass)
