...
| **Reach Zone** | Reach - sweeps a stack of points above and below working height and scores the volume of the 3D zone the gripper and leg can actually reach. | 1 |
...
The three contact tests do not use inverse kinematics: SOFA's inverse mode cannot run together with collisions.
...
Reach Zone is the exception: it has no collisions, so it runs in inverse mode and commands the effector directly around the workspace.
:::
