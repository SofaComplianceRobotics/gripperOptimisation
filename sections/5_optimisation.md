...
Why not something simpler? A **grid search** over 24 parameters with just 5 steps per axis would need $5^{24}$ evaluations - impossible. A **random search** scales fine but never learns from its results. CMA-ES sits exactly in this lab's sweet spot: a few dozen continuous parameters, noisy evaluations, no gradient.
...
When the leaderboard shows a design you like, its complete parameter set is in `runtime/trials/gen_XXXX/trial_XX/params.json`.
